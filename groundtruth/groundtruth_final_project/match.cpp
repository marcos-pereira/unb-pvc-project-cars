...
int main(int argc, char* argv[]) {
	...
	while(gts != dadosGTX.end() && running == true) {
		...
		double result = getMajor();
		if (result >= 30) {
			matchinghits++;
		}
		matches[gts->framegt-2][selecao] = result;
		selecao++;
	...
	int carsOnVideo = getCars();

	cout << "Carros no GT durante o video: " << carsOnVideo << endl;
	cout << "Carros capturados pelo metodo: " << matchinghits << endl;
	cout << "Porcentagem de acerto global: " << ((double)((double)matchinghits/(double)carsOnVideo))*100 << endl;
